...
void dpimm(uint32_t instr) {
    uint32_t sf_dpimm  = extract_bits(SF_DPIMM_HI, SF_DPIMM_LO, instr);
    uint32_t opc_dpimm = extract_bits(OPC_DPIMM_HI, OPC_DPIMM_LO, instr);
    uint32_t opi_dpimm = extract_bits(OPI_DPIMM_HI, OPI_DPIMM_LO, instr);
    uint32_t rd_dpimm  = extract_bits(RD_DPIMM_HI, RD_DPIMM_LO, instr);
    uint64_t mask = mask_from_sf(sf_dpimm);
    uint64_t entry = 0;

    if (opi_dpimm == OPI_DPIMM_ARITH) {
        uint32_t rn_arith    = extract_bits(RN_DPIMM_ARITH_HI,
                                            RN_DPIMM_ARITH_LO, instr);
        uint32_t shift_arith = extract_bits(SH_DPIMM_ARITH_HI,
                                            SH_DPIMM_ARITH_LO, instr);
        uint64_t operand2    = extract_bits(IMM12_DPIMM_HI,
                                            IMM12_DPIMM_LO, instr);
        ...
        uint32_t hw_wm = extract_bits(SH_DPIMM_WM_HI, SH_DPIMM_WM_LO,
                                      instr);
        uint64_t imm16_wm = extract_bits(IMM16_DPIMM_WM_HI,
                                         IMM16_DPIMM_WM_LO, instr);
