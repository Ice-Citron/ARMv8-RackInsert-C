...
static uint64_t compute_multiply_result(uint32_t instr, uint64_t rn, 
    ...
    uint64_t entry = 0;
    uint32_t ra_index = extract_bits(RA_DPREG_MULT_HI,
                                     RA_DPREG_MULT_LO, instr);
    uint32_t x_mult   = extract_bits(X_DPREG_MULT_HI,
                                     X_DPREG_MULT_LO, instr);
...
static uint64_t compute_logical_result(uint32_t instr, uint32_t opcode, 
    ...
    uint64_t entry = 0;
    uint64_t mask = mask_from_sf(sf);
    if (extract_bits(N_DPREG_HI, N_DPREG_LO, instr) == 1) {
...
void dpreg(uint32_t instr) {
    uint32_t sf_dpreg  = extract_bits(SF_DPREG_HI , SF_DPREG_LO , instr);
    uint32_t opc_dpreg = extract_bits(OPC_DPREG_HI, OPC_DPREG_LO, instr);
    uint32_t m_dpreg   = extract_bits(M_DPREG_HI  , M_DPREG_LO  , instr);
    uint32_t rd_index  = extract_bits(RD_DPREG_HI , RD_DPREG_LO , instr);
    uint32_t rn_index  = extract_bits(RN_DPREG_HI , RN_DPREG_LO , instr);
    uint32_t rm_index  = extract_bits(RM_DPREG_HI , RM_DPREG_LO , instr);
    ...
    if (m_dpreg == M_DPREG_MULTIPLY) {
        ...
        uint32_t opr0 = extract_bits(OPR0_DPREG_HI, OPR0_DPREG_LO, instr);
        uint32_t shift_type = extract_bits(SHIFT_DPREG_HI,
                                           SHIFT_DPREG_LO, instr);
        uint32_t shift_dist = extract_bits(SHIFT_DIST_DPREG_HI,
                                           SHIFT_DIST_DPREG_LO, instr);
